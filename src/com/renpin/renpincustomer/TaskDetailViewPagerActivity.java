...
public class TaskDetailViewPagerActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 先判断有几个图片
		for (int i = 0; i < TaskDetailActivity.strIcons.length; i++) {
			if (TaskDetailActivity.strIcons[i].length() > 100) {
				strIconsTemp[mnCount] = TaskDetailActivity.strIcons[i];
				mnCount++;
			}
		}
		...
		public View instantiateItem(ViewGroup container, int position) {
			...
			if (mUtils != null) {
				photoView.setImageBitmap(mUtils
						.base64ToBitmap(strIconsTemp[position]));
			}

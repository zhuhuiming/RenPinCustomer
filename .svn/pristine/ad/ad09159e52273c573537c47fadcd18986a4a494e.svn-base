...
public interface GoodService {
	...
	public abstract List<TaskInfoDetail> GetTaskInfoForLimit(int nType);
	...
	public abstract int SendCommentContentNewSecret(String strTaskId,
			...
			String strType, String strCommentPersonNickName,// 评论人名称(昵称)
			String strReceiveCommentPersonNickName,// 接收到评论的人名称(昵称)
			int nSecretType// 这条评论是不是悄悄话,1表示悄悄话,0表示不是
	);

	// 返回值空表示失败,其他值表示评论时间(针对多对多评论)
	...
	// 发送评论数据,返回值空表示失败,其他值表示评论时间(添加了悄悄话功能)
	...
	public abstract List<DistanceDetail> getHelpNearData(Double longitude,
			Double latitude, Double dist, String strDistrictName);

	// 获取距离小于dist的前30条数据(分享)
	public abstract List<DistanceDetail> getShareNearData(Double longitude,
			Double latitude, Double dist, String strDistrictName);

	// 获取距离大于dist的前30条数据(求助)
	public abstract List<DistanceDetail> LoadHelpNearData(Double longitude,
			Double latitude, Double dist, String strDistrictName, String strTime);

	// 获取距离大于dist的前30条数据(分享)
	public abstract List<DistanceDetail> LoadShareNearData(Double longitude,
			Double latitude, Double dist, String strDistrictName, String strTime);
	...
	// 获取动态条数(针对多对多评论,添加了系统信息),增加了任务的浏览、赞值、评论条数等数据
